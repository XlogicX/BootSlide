...
mov ah, 0xcc                           ; light-red
mov bx, 22                             ; rows
mov word [rectrow + 1], 30             ; columns
mov word [nextline + 2], 160 - (30*2)  ; put column number before '*2'
call drawrect

call loadtiles                         ; Unpack and load ordered tile numbers into memory

; Scramble
mov cl, 0xff              ; Start from solved board and do 255 random moves
scramble:
dec cx
...
gameloop:
  ...
  up:
    mov bx, [blanklocation]      ; get blank tile location
    ...
    mov [blanklocation], bx      ; update blank location
...
; This is a routine for displaying all of the tiles
  ...
  displaytilesloop:
    mov ah, 0x40          ; Black on red background
...
blanktile:
  ...
  cbw                                   ; clear ax (black)
  mov bx, 5                             ; rows
  mov word [rectrow + 1], 7             ; columns
  mov word [nextline + 2], 160 - (7*2)  ; put column number before '*2'
...
loadtiles:
; This is the unpacking routine, it takes the number data (a nibble for each number)
; and unpacks it out to bytes of data in memory space after the loaded program (0x7e00)
  xor bx, bx
  xor si, si
  mov cx, 8                   ; 8 bytes to process
  tilesloop:
    mov al, [ordered + bx]    ; Get packed byte
    and al, 0x0f              ; mask out first nibble
    mov [boarddata + si], al  ; put it into memory
    inc si                    ; increment to next byte of memory
    mov al, [ordered + bx]    ; Get same packed byte
    and al, 0xf0              ; Now get upper nibble
    shr al, 4                 ; shift it down
    mov [boarddata + si], al  ; put it into memory
    inc si                    ; increment to next byte of memory
    inc bx                    ; increment to next packed byte
    loop tilesloop
ret
...
; Color 1
mov di, dx
mov ax, 0x48b1                        ; shaded corner border
mov bx, 5                             ; rows
mov word [rectrow + 1], 7             ; columns
mov word [nextline + 2], 160 - (7*2)  ; put column number before '*2'
call drawrect
; Color 2
mov di, dx
mov ah, 0x44                          ; inner light blue main part of tile
mov bx, 4                             ; rows
mov word [rectrow + 1], 6             ; columns
mov word [nextline + 2], 160 - (6*2)  ; put column number before '*2'
call drawrect
ret
...
ordered:
dw 0xcdef, 0x89ab, 0x4567, 0x0123
...
boarddata:
; 16 bytes of board data starting at 0x7e00
